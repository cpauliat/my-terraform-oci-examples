output Instance_TENANT1 {
  ...
  ssh -i ${var.ssh_private_key_file} opc@${oci_core_instance.demo13b-tenant1.public_ip}

  ---- You can then ping instance in TENANT2 using private IP address
  ping -c 2 ${oci_core_instance.demo13b-tenant2.private_ip}
...
output Instance_TENANT2 {
  ...
  ssh -i ${var.ssh_private_key_file} opc@${oci_core_instance.demo13b-tenant2.public_ip}

  ---- You can then ping instance in TENANT1 using private IP address
  ping -c 2 ${oci_core_instance.demo13b-tenant1.private_ip}
