...
output "public_IP_adresses_of_compute_instances" {
  value = oci_core_instance.tf-demo03-ol7.*.public_ip
}
...
output "SSH_connections" {
  value = [
    for instance in oci_core_instance.tf-demo03-ol7.*:
      "ol7-${index(oci_core_instance.tf-demo03-ol7.*, instance)}: ssh -i ${var.ssh_private_key_file_ol7} opc@${instance.public_ip}"
  ]
...
output "Instances" {
  ...
  for ol7-0: ssh -i ${var.ssh_private_key_file_ol7} opc@${oci_core_instance.tf-demo03-ol7[0].public_ip}
  for ol7-1: ssh -i ${var.ssh_private_key_file_ol7} opc@${oci_core_instance.tf-demo03-ol7[1].public_ip}
  for ol7-2: ssh -i ${var.ssh_private_key_file_ol7} opc@${oci_core_instance.tf-demo03-ol7[2].public_ip}

  ---- Alternatively, you can add the following lines to your file $HOME/.ssh/config and then just run "ssh ol7-x"

  Host ol7-0
          Hostname ${oci_core_instance.tf-demo03-ol7[0].public_ip}
          User opc
          IdentityFile ${var.ssh_private_key_file_ol7}

  Host ol7-1
          Hostname ${oci_core_instance.tf-demo03-ol7[1].public_ip}
          User opc
          IdentityFile ${var.ssh_private_key_file_ol7}

  Host ol7-2
          Hostname ${oci_core_instance.tf-demo03-ol7[2].public_ip}
          User opc
          IdentityFile ${var.ssh_private_key_file_ol7}
