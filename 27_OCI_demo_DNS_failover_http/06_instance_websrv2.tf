...
resource oci_core_instance tf-demo27-ws2 {
  availability_domain  = data.oci_identity_availability_domains.ADs.availability_domains[var.AD - 1]["name"]
  compartment_id       = var.compartment_ocid
  display_name         = "tf-demo27-websrv2"
  shape                = var.shape
...
resource null_resource tf-demo27-ws2 {

  provisioner "file" {
    connection {
        agent       = false
        timeout     = "10m"
        host        = oci_core_instance.tf-demo27-ws2.public_ip
        user        = "opc"
