...
Host d07b-ws1
          Hostname ${oci_core_instance.tf-demo07b-ws1.private_ip}
          User opc
          IdentityFile ${var.ssh_private_key_file_websrv}
          StrictHostKeyChecking no
          proxycommand /usr/bin/ssh -F sshcfg -W %h:%p d07b-bastion
Host d07b-ws2
          Hostname ${oci_core_instance.tf-demo07b-ws2.private_ip}
          User opc
          IdentityFile ${var.ssh_private_key_file_websrv}
          StrictHostKeyChecking no
          proxycommand /usr/bin/ssh -F sshcfg -W %h:%p d07b-bastion
...
output "CONNECTIONS" {
  ...
  2) ---- HTTP connection to public load balancer
     ...
     http://${oci_load_balancer.tf-demo07b-lb.ip_addresses[0]}/${var.path_route_websrv1}/
     http://${oci_load_balancer.tf-demo07b-lb.ip_addresses[0]}/${var.path_route_websrv2}/
