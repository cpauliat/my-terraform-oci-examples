...
resource oci_core_instance demo39t2-test {
  ...
  create_vnic_details {
    subnet_id              = oci_core_subnet.demo39t2-public-subnet-test.id
    hostname_label         = "d39t2-test"
  }

  metadata = {
    ssh_authorized_keys = file(var.tenant2_ssh_public_key_file_ol7)
    user_data           = base64encode(file(var.tenant2_BootStrapFile_test))
  }
}