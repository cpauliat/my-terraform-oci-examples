...
# ------ Create a new VCN
resource "oci_core_virtual_network" "tf-demo20-vcn" {
...
resource "oci_core_internet_gateway" "tf-demo20-ig" {
  ...
  vcn_id         = oci_core_virtual_network.tf-demo20-vcn.id
}
...
resource "oci_core_subnet" "tf-demo20-public-subnet1" {
  ...
  dhcp_options_id     = oci_core_virtual_network.tf-demo20-vcn.default_dhcp_options_id
}
