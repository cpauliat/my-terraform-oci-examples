...
resource "oci_core_instance" "tf-demo01b-ol8" {
  availability_domain  = data.oci_identity_availability_domains.ADs.availability_domains[var.AD - 1]["name"]
  compartment_id       = var.compartment_ocid
  display_name         = "tf-demo01b-ol8"
  shape                = "VM.Standard2.1"
  preserve_boot_volume = "false"

  source_details {
    source_type = "image"
    source_id   = var.ol8_image_ocid
  }

  create_vnic_details {
    subnet_id      = oci_core_subnet.tf-demo01b-public-subnet1.id
    hostname_label = "tf-demo01b-ol8"
  }
...
output "Instance_ol8" {
  ...
  ssh -i ${var.ssh_private_key_file} ${var.new_user}@${oci_core_instance.tf-demo01b-ol8.public_ip}

  ---- Alternatively, you can add the following lines to your file $HOME/.ssh/config and then just run "ssh ol8"
  Host ol8
          Hostname ${oci_core_instance.tf-demo01b-ol8.public_ip}
          User ${var.new_user}
  ...
  https://${oci_core_instance.tf-demo01b-ol8.public_ip}    (Self signed certificate)
  user     = ${var.new_user}
