...
resource oci_core_instance tf-demo41-ol7 {
  availability_domain  = data.oci_identity_availability_domains.ADs.availability_domains[var.AD - 1]["name"]
  compartment_id       = var.compartment_ocid
  display_name         = "tf-demo41-ol7"
...
output Instance_OL7 {
  ...
  ssh -i ${var.ssh_private_key_file_ol7} opc@${oci_core_instance.tf-demo41-ol7.public_ip}

  ---- Alternatively, you can add the following lines to your file $HOME/.ssh/config and then just run "ssh ol7"

  Host ol7
          Hostname ${oci_core_instance.tf-demo41-ol7.public_ip}
          User opc
